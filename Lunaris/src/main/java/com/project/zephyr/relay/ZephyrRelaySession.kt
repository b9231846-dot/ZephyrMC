...
import com.project.zephyr.relay.handler.SessionCloseHandler
...
class ZephyrRelaySession(peer: BedrockPeer, subClientId: Int, val zephyrRelay: ZephyrRelay) {
    ...
    private fun processPacket(wrapper: BedrockPacketWrapper,
                              beforeFunc: (ZephyrRelayPacketListener) -> Boolean,
                              sendFunc: (BedrockPacket) -> Unit,
                              afterFunc: (ZephyrRelayPacketListener) -> Unit) {
        listeners.forEach {
