package com.project.zephyr.client.service
...
import com.project.zephyr.client.constructors.AccountManager
import com.project.zephyr.client.constructors.NetBound
import com.project.zephyr.client.constructors.GameManager
import com.project.zephyr.client.model.CaptureModeModel
...
import com.project.zephyr.relay.listener.AutoCodecPacketListener
...
class Services : Service() {
    ...
    companion object {
        const val ACTION_CAPTURE_START = "com.project.zephyr.relay.capture.start"
        const val ACTION_CAPTURE_STOP = "com.project.zephyr.relay.capture.stop"
        private const val NOTIFICATION_CHANNEL_ID = "zephyr_capture_channel"
        private const val NOTIFICATION_ID = 1001

        private val handler = Handler(Looper.getMainLooper())
        private var ZephyrRelay: ZephyrRelay? = null
        ...
        private fun on(context: Context, captureModeModel: CaptureModeModel) {
            ...
            val currentActivity = activityManager.appTasks
                ...
                .firstOrNull()
            RemisOnline = currentActivity == "com.project.zephyr.client.activity.RemoteLinkActivity"
            ...
            thread = thread(name = "ZephyrRelayThread") {
                runCatching {
                ...
                val sessionEncryptor = if (AccountManager.currentAccount == null) {
                    ...
                    AccountManager.currentAccount?.let { account ->
                        Log.e("ZephyrRelay", "Logged in as ${account.remark}")
                ...
                runCatching {
                    ZephyrRelay = captureZephyrRelay(
                    ...
                    it.printStackTrace()
                    context.toast("Start ZephyrRelay error: ${it.stackTraceToString()}")
                }
            }
        }

        private fun off() {
            thread(name = "ZephyrRelayThread") {
                GameManager.saveConfig()
                isActive = false
                RemisOnline = false
                currentServerHostName = ""
                PresenceStateManager.onRelayDisconnected()
                ZephyrRelay?.disconnect()
                thread?.interrupt()
                thread = null
        ...
        private fun initModules(ZephyrRelaySession: ZephyrRelaySession) {
            try {
                val session = NetBound(ZephyrRelaySession)
                ZephyrRelaySession.listeners.add(session)
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_CAPTURE_START -> {
                startForeground(NOTIFICATION_ID, createNotification("Zephyr capture service is running"))
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                "Zephyr Capture Service",
                NotificationManager.IMPORTANCE_LOW
            ).apply {
                description = "Used while Zephyr capture mode is active"
            }
    ...
    private fun createNotification(text: String) = NotificationCompat.Builder(this, NOTIFICATION_CHANNEL_ID)
        .setContentTitle("Zephyr Capture")
        .setContentText(text)
